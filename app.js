    ...
    /* =========================================================
       AYON AI KNOWLEDGE MANAGER — FINAL FRONTEND CONTROL
       Keep this section at the end of app.js.
    ========================================================= */
    ...
    async function loadAyonKnowledge({ quiet = false } = {}) {
      if (!session || !isManager()) return false;
      if (!quiet) setBusy(true, 'Loading AYON AI Knowledge…');
      try {
        const r = await apiPost('aiKnowledge', {});
        if (!r?.ok) throw new Error(r?.error || 'Knowledge load failed');
        ayonKnowledge = Array.isArray(r.data) ? r.data : [];
        return true;
      } catch (e) {
        console.warn('AYON Knowledge', e);
        if (!quiet) toast(e.message || 'Knowledge load failed', 3500);
        return false;
      } finally {
        if (!quiet) setBusy(false);
      }
    ...
    function ayonToneLabel(v) {
      const x = String(v || 'NORMAL').toUpperCase();
      return x === 'STRICT-FUNNY' ? 'Strict-Funny'
        : x === 'MOTIVATIONAL' ? 'Motivational'
        : x === 'FUNNY' ? 'Funny' : 'Normal';
    }
    ...
    function renderAyonKnowledge() {
      ...
      const edit = editingAyonKnowledgeId
        ? ayonKnowledge.find(x => String(x.id) === String(editingAyonKnowledgeId))
        : null;

      $('#mainContent').innerHTML = `${monthBar({ showManager:false })}
        <section class="hero">
          <p class="eyebrow">MANAGER • AYON AI</p>
          <h3>Knowledge Manager</h3>
          <p class="muted">Teach AYON without editing code. Knowledge answers are checked before Sales Coach and live database routing.</p>
          <div class="form-actions">
            <button id="ayonKbRefresh" class="btn secondary">↻ REFRESH KNOWLEDGE</button>
            <button id="ayonKbNew" class="btn primary">＋ NEW ANSWER</button>
          </div>
        </section>

        <div class="card" style="margin-top:12px">
          <p class="eyebrow">${edit ? 'EDIT KNOWLEDGE' : 'ADD KNOWLEDGE'}</p>
          <form id="ayonKbForm" class="stack">
            <label>Question / Keywords
              <textarea name="question" required placeholder="Example: outlet-এ order না দিলে কী করব? | buyer handling | no order">${esc(edit?.question || '')}</textarea>
            </label>
            <label>Answer / Instruction
              <textarea name="answer" required style="min-height:130px" placeholder="Write exactly how AYON should answer.">${esc(edit?.answer || '')}</textarea>
            </label>
            <div class="form-grid">
              <label>Tone
                <select name="tone">
                  <option value="Normal" ${ayonToneLabel(edit?.tone)==='Normal'?'selected':''}>Normal</option>
                  <option value="Funny" ${ayonToneLabel(edit?.tone)==='Funny'?'selected':''}>Funny</option>
                  <option value="Motivational" ${ayonToneLabel(edit?.tone)==='Motivational'?'selected':''}>Motivational</option>
                  <option value="Strict-Funny" ${ayonToneLabel(edit?.tone)==='Strict-Funny'?'selected':''}>Strict-Funny</option>
                </select>
              </label>
              <label>Status
                <select name="active">
                  <option value="true" ${edit?.active===false?'':'selected'}>Active</option>
                  <option value="false" ${edit?.active===false?'selected':''}>Inactive</option>
                </select>
              </label>
            </div>
            <div class="form-actions">
              <button class="btn primary">${edit ? 'UPDATE KNOWLEDGE' : 'SAVE KNOWLEDGE'}</button>
              ${edit ? '<button type="button" id="ayonKbCancel" class="btn secondary">CANCEL EDIT</button>' : ''}
            </div>
          </form>
        </div>

        <div class="section-title"><h3>Saved Knowledge</h3></div>
        <div class="list">
          ${ayonKnowledge.length ? ayonKnowledge.map((x,i) => `
            <div class="list-item" style="${x.active===false?'opacity:.62':''}">
              <div class="row">
                <div style="min-width:0">
                  <span class="pill ${x.active===false?'red':'green'}">${x.active===false?'INACTIVE':'ACTIVE'}</span>
                  <span class="pill orange">${esc(ayonToneLabel(x.tone))}</span>
                  <h4 style="margin-top:8px">${i+1}. ${esc(x.question || '')}</h4>
                  <p style="white-space:pre-wrap">${esc(x.answer || '')}</p>
                </div>
              </div>
              <div class="form-actions">
                <button class="btn secondary" data-ayonkbedit="${esc(x.id)}">EDIT</button>
                ${x.active===false
                  ? `<button class="btn primary" data-ayonkbrestore="${esc(x.id)}">RESTORE</button>`
                  : `<button class="btn danger" data-ayonkbdelete="${esc(x.id)}">DEACTIVATE</button>`}
              </div>
            </div>`).join('') : empty('No Manager Knowledge has been added yet.')}
        </div>`;
      ...
      if ($('#ayonKbCancel')) $('#ayonKbCancel').onclick = () => {
        editingAyonKnowledgeId = null;
        renderAyonKnowledge();
      };
      ...
      $('#ayonKbForm').onsubmit = async e => {
        ...
        setBusy(true, edit ? 'Updating AYON knowledge…' : 'Saving AYON knowledge…');
        try {
          ...
          if (!r?.ok) throw new Error(r?.error || 'Knowledge save failed');
          editingAyonKnowledgeId = null;
          await loadAyonKnowledge({ quiet:true });
          toast(edit ? 'AYON knowledge updated' : 'AYON learned the new answer');
          renderAyonKnowledge();
        } catch (x) {
      ...
      $$('[data-ayonkbedit]').forEach(b => b.onclick = () => {
        editingAyonKnowledgeId = b.dataset.ayonkbedit;
        renderAyonKnowledge();
        window.scrollTo({top:0,behavior:'smooth'});
      });

      $$('[data-ayonkbdelete]').forEach(b => b.onclick = async () => {
        if (!confirm('Deactivate this AYON knowledge answer?')) return;
        setBusy(true, 'Deactivating knowledge…');
        try {
          const r = await apiPost('deleteAiKnowledge', { id:b.dataset.ayonkbdelete });
      ...
      $$('[data-ayonkbrestore]').forEach(b => b.onclick = async () => {
        setBusy(true, 'Restoring knowledge…');
        try {
          const r = await apiPost('restoreAiKnowledge', { id:b.dataset.ayonkbrestore });
    ...
    /* Final Settings screen: Manager gets direct AYON Knowledge Manager access. */
    function renderSettings() {
      const ayonManager = isManager()
        ? `<div class="card" style="margin-top:12px">
            <p class="eyebrow">MANAGER AI CONTROL</p>
            <h3>AYON AI Knowledge Manager</h3>
            <p class="muted">Add, edit, deactivate or restore AYON answers without changing code.</p>
            <button id="openAyonKnowledge" class="btn primary" style="width:100%">OPEN KNOWLEDGE MANAGER</button>
          </div>` : '';

      $('#mainContent').innerHTML = `${monthBar()}
        <section class="hero"><p class="eyebrow">SETTINGS</p><h3>Account & App</h3>
        <p class="muted">This phone stays logged in until you press Logout.</p>${syncStatus()}</section>
        ${ayonManager}
        <div class="card" style="margin-top:12px">
          <div class="list-item"><h4>Signed in</h4><p>${esc(sessionName())} • ${esc(session.id)} • ${esc(String(session.mode).toUpperCase())}</p></div>
          <div class="list-item" style="margin-top:8px"><h4>Backend</h4><p>${backendUrl() ? 'Connected' : 'Missing'} • Server ${esc(current?.version || '—')}</p></div>
          <button id="settingsSync" class="btn secondary" style="margin-top:12px;width:100%">↻ SYNC LIVE DATA NOW</button>
          <button id="realLogout" class="btn danger" style="margin-top:10px;width:100%">LOG OUT</button>
        </div>
        <div class="card" style="margin-top:12px;text-align:center"><p class="muted" style="margin:0">Developed by <a href="https://mehedialimayon-del.github.io/MEHEDI-ALIM-AYON-PORTFOLIO/" target="_blank" rel="noopener" style="color:#ff7414;text-decoration:none;font-weight:900">KAM AYON</a></p></div>`;

      bindCommon();
      if ($('#openAyonKnowledge')) $('#openAyonKnowledge').onclick = async () => {
        page = 'aiKnowledge';
        render();
        await loadAyonKnowledge({ quiet:true });
        render();
    ...
    /* Final render router including Manager AI Knowledge. */
    function render() {
      if (!session) return;
      installShell(); refreshTop(); bindNav();
      const map = {
...
/* =========================================================
   FINAL LAUNCH V8 UI — TEAM / DYNAMIC SR / ALL SR
   Compatible with Code.gs FINAL-LAUNCH-V8-2026.09.18
========================================================= */
let v8SRs=[], v8Team=null, v8Outlets=[];
async function v8LoadManagerData(){
  if(!isManager()) return;
  try{
    const [s,t]=await Promise.all([apiPost('activeSRs',{}),apiPost('teamDashboard',{month:selectedMonth,date:selectedDate})]);
    if(s?.ok)v8SRs=s.data||[]; if(t?.ok)v8Team=t.data||null;
  }catch(e){console.warn('V8 manager data',e)}
}
function v8SrOptions(includeAll=true,includeManager=true){
  let h=includeAll?'<option value="ALL">ALL SR — MY TEAM</option>':'';
  if(includeManager)h+=`<option value="${esc(session.id)}">MYSELF — ${esc(sessionName())}</option>`;
  h+=v8SRs.map(x=>`<option value="${esc(x.staffId)}">${esc(x.name)} • ${esc(x.staffId)}${x.route?' • '+esc(x.route):''}</option>`).join('');return h;
}
function v8Kpi(label,value,cls=''){return `<div class="metric-box"><small>${esc(label)}</small><br><b class="${cls}">${value}</b></div>`}
function renderDashboard(){
  if(!isManager()) return renderDashboardV8SR();
  if(!v8Team){$('#mainContent').innerHTML=`${monthBar({showManager:false})}<section class="hero"><p class="eyebrow">MANAGER • ALL SR</p><h3>Loading Team Dashboard…</h3>${syncStatus()}</section>`;v8LoadManagerData().then(render);return}
  const x=v8Team.performance||{},members=v8Team.members||[];
  $('#mainContent').innerHTML=`${monthBar({showManager:false})}<section class="hero"><p class="eyebrow">MANAGER • ALL SR — MY TEAM</p><h3>Full Zone Dashboard</h3><p class="muted">${members.length} active SR • delivered sales source of truth</p>${syncStatus()}<button id="v8Refresh" class="btn primary">↻ SYNC TEAM NOW</button></section>
  <div class="mini-grid" style="margin-top:12px">${v8Kpi('Team Target',money(x.target))}${v8Kpi('Delivered',money(x.achievement),'good')}${v8Kpi('Achievement',pct(x.percent),x.percent>=100?'good':'')}${v8Kpi('Shortfall',money(x.shortfall),'bad')}${v8Kpi('Today Sales',money(x.todaySales))}${v8Kpi('Pending Delivery',n(v8Team.pendingOrders),'bad')}${v8Kpi('Coverage',pct(x.coverage))}${v8Kpi('Zero Sales',n(x.zeroOutlets),'bad')}${v8Kpi('Pending CPO',n(v8Team.pendingCpo),'bad')}${v8Kpi('Pending Tasks',n(v8Team.pendingTasks),'bad')}</div>
  <div class="section-title"><h3>Individual SR Drill-down</h3></div><div class="list">${members.map(m=>{const p=m.performance||{};return `<div class="list-item"><div class="row"><div><h4>${esc(m.name)}</h4><p>${esc(m.staffId)} • ${esc(m.route||'')} • ${money(p.achievement)} / ${money(p.target)}</p></div><span class="pill ${p.percent>=100?'green':p.percent>=80?'orange':'red'}">${pct(p.percent)}</span></div><div class="form-actions"><button class="btn action-sales" data-v8open="${esc(m.staffId)}" data-page="dashboard">DASHBOARD</button><button class="btn secondary" data-v8open="${esc(m.staffId)}" data-page="summary">FULL DATA</button><button class="btn action-plan" data-v8open="${esc(m.staffId)}" data-page="planning">PLAN</button></div></div>`}).join('')}</div>`;
  bindCommon();$('#v8Refresh').onclick=async()=>{setBusy(true,'Syncing full team…');await v8LoadManagerData();setBusy(false);render()};$$('[data-v8open]').forEach(b=>b.onclick=async()=>{managerView=b.dataset.v8open;session.managerView=managerView;saveSession();await loadCurrent();setPage(b.dataset.page||'summary')});
}
const renderDashboardV8SR = (()=>{ const f=typeof renderDashboard==='function'?null:null; return function(){
  const p=current?.performance||{},a=alertsForCurrent();$('#mainContent').innerHTML=`${monthBar()}<section class="hero"><p class="eyebrow">${esc(viewedName())}</p><h3>My Sales Dashboard</h3><p class="muted">Today mission: sales update • important work • pending delivery • CPO</p>${syncStatus()}</section><div class="mini-grid" style="margin-top:12px">${v8Kpi('Target',money(p.target))}${v8Kpi('Delivered',money(p.achievement),'good')}${v8Kpi('Achievement',pct(p.percent))}${v8Kpi('Shortfall',money(p.shortfall),'bad')}${v8Kpi('Today Sales',money(p.todaySales))}${v8Kpi('Zero Sales',n(p.zeroOutlets),'bad')}</div><div class="section-title"><h3>Today Mission</h3></div><div class="list">${a.length?a.map(x=>`<div class="list-item"><h4>${esc(x.title)}</h4><p>${esc(x.text||'')}</p></div>`).join(''):empty('No urgent action right now.')}</div>`;bindCommon();
}})();

function renderTasks(){
  const tasks=current?.tasks||[], manager=isManager();
  if(manager&&!v8SRs.length){v8LoadManagerData().then(render)}
  const form=`<div class="card" style="margin-top:12px"><p class="eyebrow">${manager?'ASSIGN / PERSONAL REMINDER':'MY REMINDER'}</p><form id="taskFormV8" class="stack"><label>Assign To<select name="staffId">${manager?v8SrOptions(true,true):`<option value="${esc(session.id)}">MYSELF</option>`}</select></label><label>Title<input name="title" required placeholder="Buyer meeting / follow-up / important work"></label><label>Instruction / Note<textarea name="instruction"></textarea></label><div class="form-grid"><label>Due Date<input name="due" type="date" value="${selectedDate}"></label><label>Due Time<input name="dueTime" type="time"></label></div><div class="form-grid"><label>Type<select name="source"><option value="OWN">My Reminder</option><option value="MANAGER">Manager Work</option><option value="MEETING">Meeting</option><option value="BUYER">Buyer Follow-up</option></select></label><label>Priority<select name="priority"><option>NORMAL</option><option>HIGH</option><option>URGENT</option></select></label></div><button class="btn primary">SAVE & ENABLE PUSH REMINDER</button></form></div>`;
  $('#mainContent').innerHTML=`${monthBar()}<section class="hero"><p class="eyebrow">IMPORTANT WORK</p><h3>Tasks, Meetings & Personal Reminders</h3><p class="muted">Personal reminder stays with you. Manager can assign one SR or ALL SR independently.</p></section>${form}<div class="section-title"><h3>${manager?'Selected User / My Work':'My Work'}</h3></div><div class="list">${tasks.length?tasks.map(t=>`<div class="list-item"><div class="row"><div><h4>${esc(t.Title||t.title)}</h4><p>${esc(t.Instruction||t.instruction||'')}<br>${esc(t['Due Date']||t.due||'')} ${esc(t['Due Time']||'')} • ${esc(t.Source||'OWN')}</p></div><span class="pill ${taskDone(t)?'green':'orange'}">${taskDone(t)?'DONE':'PENDING'}</span></div>${!taskDone(t)?`<button class="btn primary" data-taskdone="${esc(t['Task ID']||t.id)}" data-tasktitle="${esc(t.Title||t.title||'')}">COMPLETE</button>`:''}</div>`).join(''):empty('No task/reminder available.')}</div>`;
  bindCommon();$('#taskFormV8').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.target),sid=String(fd.get('staffId'));setBusy(true,'Saving important work…');try{const r=await apiPost('saveTask',{staffId:sid,scope:sid==='ALL'?'ALL':'STAFF',title:String(fd.get('title')),instruction:String(fd.get('instruction')),due:String(fd.get('due')),dueTime:String(fd.get('dueTime')),source:String(fd.get('source')),priority:String(fd.get('priority')),reminderEnabled:true});if(!r?.ok)throw new Error(r?.error||'Task save failed');if(sid!=='ALL'&&sid!==session.id&&manager){managerView=sid;await loadCurrent({quiet:true})}else await loadCurrent({quiet:true});toast(sid==='ALL'?'Task created separately for ALL active SR':'Reminder / task saved');render()}catch(x){toast(x.message,4000)}finally{setBusy(false)}};$$('[data-taskdone]').forEach(b=>b.onclick=async()=>{setBusy(true,'Completing…');try{const r=await apiPost('completeTask',{taskId:b.dataset.taskdone});if(!r?.ok)throw new Error(r?.error||'Failed');await loadCurrent({quiet:true});toast('Completed');render()}catch(x){toast(x.message,4000)}finally{setBusy(false)}});
}

function renderZero(){
  if(isManager()){
    $('#mainContent').innerHTML=`${monthBar({showManager:false})}<section class="hero"><p class="eyebrow">ZONE OUTLET CONTROL</p><h3>ALL / Zero Sales Outlets</h3><div class="form-grid"><button class="btn primary" id="v8AllOut">ALL OUTLETS</button><button class="btn danger" id="v8ZeroOut">ZERO SALES ONLY</button></div></section><div id="v8OutletList" class="list" style="margin-top:12px">${empty('Choose ALL or ZERO SALES.')}</div>`;bindCommon();const load=async mode=>{setBusy(true,'Loading zone outlets…');try{const r=await apiPost('teamOutlets',{month:selectedMonth,mode});if(!r?.ok)throw new Error(r?.error||'Load failed');v8Outlets=r.data||[];$('#v8OutletList').innerHTML=v8Outlets.length?v8Outlets.map(o=>`<div class="list-item"><div class="row"><div><h4>${esc(o.outletName)}</h4><p>${esc(o.staffName)} • ${esc(o.staffId)} • ${esc(o.route||'')}</p></div><span class="pill ${o.sales>0?'green':'red'}">${o.sales>0?money(o.sales):'ZERO'}</span></div></div>`).join(''):empty('No matching outlet.') }catch(x){toast(x.message,4000)}finally{setBusy(false)}};$('#v8AllOut').onclick=()=>load('ALL');$('#v8ZeroOut').onclick=()=>load('ZERO');return;
  }
  const rows=current?.outletSummary||[];$('#mainContent').innerHTML=`${monthBar()}<section class="hero"><p class="eyebrow">MY OUTLETS</p><h3>Outlet Sales / Zero Sales</h3></section><div class="list">${rows.map(o=>`<div class="list-item"><div class="row"><div><h4>${esc(o.outletName)}</h4><p>${esc(o.outletCode||'')}</p></div><span class="pill ${n(o.sales)>0?'green':'red'}">${n(o.sales)>0?money(o.sales):'ZERO'}</span></div></div>`).join('')}</div>`;bindCommon();
}

const renderSettingsV8Base=renderSettings;
function renderSettings(){
  renderSettingsV8Base(); if(!isManager())return; const root=$('#mainContent');if(!root)return;
  root.insertAdjacentHTML('beforeend',`<div class="card" style="margin-top:12px"><p class="eyebrow">TEAM MANAGEMENT</p><h3>Add / Edit / Activate SR</h3><p class="muted">USERS sheet is the source of truth. New active SR automatically appears in ALL SR selectors.</p><form id="v8UserForm" class="stack"><div class="form-grid"><label>Staff ID<input name="staffId" required></label><label>Full Name<input name="name" required></label></div><div class="form-grid"><label>Route / Area<input name="route"></label><label>Role<select name="role"><option value="SR">SR</option><option value="MANAGER">MANAGER</option></select></label></div><label>Initial PIN / New PIN<input name="password" type="password" placeholder="Leave blank when not changing"></label><button class="btn primary">SAVE USER</button></form><div class="section-title"><h3>Active SR</h3></div><div id="v8Users">${v8SRs.length?v8SRs.map(u=>`<div class="list-item"><div class="row"><div><h4>${esc(u.name)}</h4><p>${esc(u.staffId)} • ${esc(u.route||'')}</p></div><button class="btn danger" data-v8deactivate="${esc(u.staffId)}">DEACTIVATE</button></div></div>`).join(''):empty('Loading active SR…')}</div></div>`);
  if(!v8SRs.length)v8LoadManagerData().then(renderSettings);$('#v8UserForm').onsubmit=async e=>{e.preventDefault();const fd=new FormData(e.target);setBusy(true,'Saving user…');try{const r=await apiPost('saveUser',{staffId:String(fd.get('staffId')),name:String(fd.get('name')),route:String(fd.get('route')),area:String(fd.get('route')),role:String(fd.get('role')),password:String(fd.get('password')),active:true});if(!r?.ok)throw new Error(r?.error||'Save failed');await v8LoadManagerData();toast('User saved');renderSettings()}catch(x){toast(x.message,4000)}finally{setBusy(false)}};$$('[data-v8deactivate]').forEach(b=>b.onclick=async()=>{if(!confirm('Deactivate this SR?'))return;const r=await apiPost('setUserActive',{staffId:b.dataset.v8deactivate,active:false});if(r?.ok){await v8LoadManagerData();toast('SR deactivated');renderSettings()}else toast(r?.error||'Failed')});
}
