...
(() => {
  ...
  function isLiveDataQuestion(text) {
    const q = normalizeText(text);
    const metric = [
      'target','টার্গেট','shortfall','শর্টফল','achievement','অ্যাচিভ','sales','sale','সেল',
      'delivered','delivery','ডেলিভারি','pending','পেন্ডিং','zero','জিরো','শূন্য',
      'incentive','ইনসেনটিভ','task','important work','কাজ','cpo','income','salary',
      'commission','ইনকাম','কমিশন','today sales','আজকের সেল','coverage','কভারেজ'
    ];
    const team = [
      'emon','ইমন','badrud','বদরুদ্দোজা',
      'limon','লিমন','majumder','মজুমদার',
      'munnaf','মুন্নাফ','মুন্নফ',
      'adib','আদিব','rubayat','রুবায়াত','রুবায়াত',
      'team','সব salesman','সব সেলসম্যান','সব sr','all sr'
    ];
    const explicitLive = [
      'live data','লাইভ ডাটা','database','ডাটাবেজ','কত sale','কত সেল','কত বাকি',
      'আজ কত','today কত','status','স্ট্যাটাস','report','রিপোর্ট'
    ];
    return has(q, explicitLive) || (has(q, metric) && (has(q, team) || /\b\d/.test(q)));
  }

  async function backendAyonAnswer(text) {
    if (typeof apiPost !== 'function' || !getSession()?.id) return null;
    ...
    try {
      const payload = {
        ...
        date: todayKey(),
        viewStaffId: (() => {
          try { return typeof viewedId === 'function' ? viewedId() : getSession()?.id; }
          catch { return getSession()?.id; }
        })(),
        liveData: isLiveDataQuestion(text)
      };

      const actions = ['askAI','askAi','aiAsk','askAyon','ayonAI','aiChat'];
      let lastError = '';
      ...
      for (const action of actions) {
        try {
          ...
          const answer =
            r.answer ?? r.reply ?? r.message ?? r.text ??
            r.data?.answer ?? r.data?.reply ?? r.data?.message ?? r.data?.text;
      ...
      if (lastError) console.warn('AYON backend fallback:', lastError);
      return null;
    } catch (e) {
      console.warn('AYON backend unavailable:', e);
      return null;
    }
  }
  ...
  async function answerQuestion(text) {
    ...
    /*
      ...
      2) Backend handles explicit live/team questions with permission control.
      3) If backend is unavailable or has no answer, existing Human Sales Coach
         remains available locally, so normal conversation stays fast.
    */
    const cloud = await backendAyonAnswer(text);
  ...
  function injectUI() {
    ...
    panel.innerHTML = `
      ...
      <div class="aa-compose">
        ...
        <div class="aa-foot">
          <span>${BUILD} • Manager KB + Sales Coach</span>
  ...
  function ask(text, fromVoice = false) {
    ...
    setTimeout(async () => {
      try {
        ...
        typing?.remove();
        const reply = localAnswerQuestion(text);
        addMessage('ai', reply);
        if (autoVoice && (fromVoice || opened)) speak(reply, text);
      }
    }, 120);
